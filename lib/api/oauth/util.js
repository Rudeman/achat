...
var max_timestamp_delta = 6000; // todo !!
...
//            console.log('store:\n' + sys.inspect(store));
            ...
            if (token) {
                ...
                if (consumer) {
                    var signature = params.oauth_signature;
                    var url = 'http://' + req.headers.host +  URL.parse(req.url).pathname;
                    var sig2 = signParams(req.method, url,
...
function generateKeypair(generatorKey, string, algorithm) {
...
function generateConsumerCredensials (consumer, algorithm) {
    var keyPair = generateKeypair(consumer.oauthNonce, 
        consumer.name+consumer.description+consumer.url+consumer.callbackUrl+consumer.osType, 
        algorithm);
    consumer.key = keyPair.key;
    consumer.secret = keyPair.secret;
    consumer.timeStamp = (new Date()).getTime()/1000 + max_timestamp_delta;
    
    return consumer;
}

function generateRequestToken (oauthParams, consumer, algorithm) {
    var tok = generateKeypair(oauthParams.oauthNonce, consumer.secret +
                              oauthParams.oauth_signature, algorithm);
...
function generateSkipAccessToken(consumer, algorithm) {
    var tok = generateKeypair(consumer.secret+consumer.nonce,
                              consumer.name + consumer.description+consumer.secret+
                              consumer.callbackUrl+consumer.osType, algorithm);
    tok.timeStamp = (new Date()).getTime()/1000 + max_timestamp_delta;
    
    return tok;
}

function generateAccessToken(requestToken, algorithm) {
...
exports.generateConsumerCredensials = generateConsumerCredensials;
exports.generateRequestToken = generateRequestToken;
exports.generateAccessToken = generateAccessToken;
exports.generateSkipAccessToken = generateSkipAccessToken;
