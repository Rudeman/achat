    ...
    function postChat(req,res,data,action) {
        ...
        database.runQuery(query,function(error,records,sentQuery){
            ...
            database.runQuery(query,function(error,records,sentQuery){
                ...
                var query = "SELECT User.id, User.login, User.displayName, User.regId FROM User, UserRoom " +
                    "WHERE User.regId IS NOT NULL " +
                    "AND UserRoom.roomId = "+data.id+" " +
                    "AND UserRoom.userId = User.id";
                database.runQuery(query, function(error,records) {
                    ...
                    for(var i in records) {
                        ...
                        if(records[i].id == data.tokenUserId) {
                            if( records[i].displayName == null || records[i].displayName == "" ) {
