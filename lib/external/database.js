...
function DBQueryHandler (query, handler) {
    this.query = query;
    this.handler = handler;
    
    var self = this;
    this.onQueryComplete = function(error,records) {
        if(self.handler)
            self.handler(error,records,self.query);
    }
}
...
function makeQuery(db, query, handler, endHandler) {
    ...
    for(var i=0; i<queries.length; i++) {
        if(queries[i] != "" && queries[i] != null) {
            var queryHandler = new DBQueryHandler(queries[i],function (error, records, subQuery) {
                query_count--;
                ...
                if(handler !== null) {
                    handler(error,records, subQuery);
                }
                
                if( query_count <= 0 && endHandler != null ) {
                    endHandler(error,records, subQuery);
                }
            });

            db.all(queries[i], queryHandler.onQueryComplete);
        }
        else query_count--;
    }
}
